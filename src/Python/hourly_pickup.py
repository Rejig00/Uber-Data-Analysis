...
query_table = pd.DataFrame()
for hours in range(1, 24, 1):
     query_table = query_table.append(df.query('pickup_dt == ["' + str(hours) + '"] and borough == ["NA"]'))
plt.scatter(query_table["pickup_dt"], query_table["pickups"], alpha=0.1, color='grey', label='NA')
z = np.polyfit(query_table["pickup_dt"], query_table["pickups"], 3)
p = np.poly1d(z)
plt.plot(query_table["pickup_dt"], p(query_table["pickup_dt"]),"grey") 
