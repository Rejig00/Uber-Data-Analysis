...
ggplot(df, aes(x=pickup_dt, y=pickups, col=borough))%>%
  +geom_smooth(se=FALSE, col='black')%>%
  +geom_point(alpha=0.075)%>%
  +stat_summary(geom = "line", fun = "mean", col='black')%>%
  +stat_summary(geom = "line", fun = "quantile", fun.args = list(probs=0.25), col='blue', linetype=2)%>%
  +stat_summary(geom = "line", fun = "quantile", fun.args = list(probs=0.75), col='blue', linetype=2)%>%
  +facet_wrap(~borough, scales = 'free')
ggplot(query_table, aes(x=pickup_dt, y=pickups))%>%
